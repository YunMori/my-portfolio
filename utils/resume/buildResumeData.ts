...
// 이 구조가 그대로 resume_versions.snapshot 에 저장된다 — 별도 직렬화 계층이 없으므로
// 필드를 바꿀 때는 이미 저장된 스냅샷도 읽을 수 있는지(=필드 추가만) 고려해야 한다.
...
// 저장된 버전의 selections를 최신 아카이브 위에 복원한다.
// 그 사이 삭제된 항목의 id는 조용히 걸러내므로, 복원 결과는 항상 현재 데이터와 정합적이다.
// (버전의 snapshot은 이 함수를 거치지 않는다 — 스냅샷은 저장 시점 그대로 렌더링된다.)
